...
local function raise_on_color_changed_event(container, color, player_index)
	game.raise_event(on_color_changed_event_id, {container = container, color = color, player_index = player_index})
end
...
local function raise_on_color_changed_event_255(container, r, g, b, player_index)
	raise_on_color_changed_event(container, {r = r / 255, g = g / 255, b = b / 255}, player_index)
end
...
local function set_color_255(color_picker_frame, r, g, b)
	...
	result_label.style.font_color = {r = r / 255, g = g / 255, b = b / 255}
end
...
local function set_color(color_picker_container, color)
	...
	assert(frame, "Make sure the correct color picker container is provided.")
...
local function create_rgb_bar_contents(bar_container, element_name_prefix, label_caption)
	bar_container.add{type = "table", name = element_name_prefix .. color_picker_defines.names.gui.rgb_bar_label_container_postfix, colspan = 2}
	local label_container = bar_container[element_name_prefix .. color_picker_defines.names.gui.rgb_bar_label_container_postfix]
	label_container.style.minimal_width = 40
	label_container.style.maximal_width = 40
	label_container.style.horizontal_spacing = 0
	label_container.style.top_padding = 4
	...
	bar_container.add{type = "table", name = element_name_prefix .. color_picker_defines.names.gui.rgb_bar_button_table_postfix, colspan = 16}
	local button_table = bar_container[element_name_prefix .. color_picker_defines.names.gui.rgb_bar_button_table_postfix]
	button_table.style.horizontal_spacing = 0
	button_table.style.top_padding = 8
	for i = 0, 15, 1 do
...
local function add_instance(data)
	...
	parent.add{type = "flow", name = data.container_name, direction = "vertical"}
	local container = parent[data.container_name]
	
	-- Frame.
	container.add{type = "frame", name = color_picker_defines.names.gui.frame, direction = "vertical", caption = data.title_caption or {"gui-train.color"}}
	local frame = container[color_picker_defines.names.gui.frame]
	
	-- Color palettes.
	frame.add{type = "table", name = color_picker_defines.names.gui.color_palette_table, colspan = 13}
	local color_palette_table = frame[color_picker_defines.names.gui.color_palette_table]
	color_palette_table.style.left_padding = 25
	-- color_palette_table.style.cell_spacing = 0 -- Don't know why it doesn't work.
	color_palette_table.style.horizontal_spacing = 0
	color_palette_table.style.vertical_spacing = 0
	-- Black palettes.
	...
	-- Red bar.
	frame.add{type = "flow", name = color_picker_defines.names.gui.red_bar_container, direction = "horizontal"}
	local bar_container = frame[color_picker_defines.names.gui.red_bar_container]
	...
	-- Green bar.
	frame.add{type = "flow", name = color_picker_defines.names.gui.green_bar_container, direction = "horizontal"}
	bar_container = frame[color_picker_defines.names.gui.green_bar_container]
	create_rgb_bar_contents(bar_container, color_picker_defines.names.gui.green_bar_contents_prefix, {"green", ":"})
	
	-- Blue bar.
	frame.add{type = "flow", name = color_picker_defines.names.gui.blue_bar_container, direction = "horizontal"}
	bar_container = frame[color_picker_defines.names.gui.blue_bar_container]
	...
	frame.add{type = "flow", name = color_picker_defines.names.gui.result_container, direction = "horizontal"}
	local result_container = frame[color_picker_defines.names.gui.result_container]
	result_container.style.top_padding = 16
	-- Result label.
	result_container.add{type = "label", name = color_picker_defines.names.gui.result_label, caption = data.result_caption or {"gui.color-picker_result-text"}}
	local result_label = result_container[color_picker_defines.names.gui.result_label]
	if show_ok_button then
		result_label.style.top_padding = 10
		result_label.style.minimal_width = 15 * 13 + 25 * 2 - 60
	else
		result_label.style.minimal_width = 15 * 13 + 25 * 2
	end
	result_label.style.maximal_width = result_label.style.minimal_width
	-- OK button.
	if show_ok_button then
		result_container.add{type = "button", name = color_picker_defines.names.gui.ok_button, caption = {"gui.ok"}}
		local ok_button = result_container[color_picker_defines.names.gui.ok_button]
		ok_button.style.minimal_width = 60
		ok_button.style.maximal_width = ok_button.style.minimal_width
	end
	
	-- Set color.
	set_color(container, data.color or {r = 1, g = 1, b = 1})
				...
				if container then
					...
					if r_bar_container and g_bar_container and b_bar_container then
						...
						if r_textfield and g_textfield and b_textfield then
							...
							game.raise_event(on_ok_button_clicked_event_id, {container = container, color = color, player_index = player_index})
							return
						end
					end
				end
...
remote.add_interface("color-picker",
{
	...
	--  		color :: Color (optional): The color this color picker is representing. If not provided, {r = 1, g = 1, b = 1} will be used. The value of a will be ignored if provided.
